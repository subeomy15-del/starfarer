...
A desktop 3D exploration game with six universes, 24 planets per universe (144 total), large crewed stations and walkable landing sites, orbital combat, collectibles, five ship upgrade types, and six universe guardians.
...
Every session and rescue begins on foot at your universe's Ark station. The station connects a hangar, command center, science lab, crew quarters, medical bay, and observation cupola through pressurized tunnels. Its orbital exterior has linked habitation modules and large solar wings. Read the opening chapter, walk down the lit hangar path, approach the Kestrel, and press **E** to board. The ship lifts off and exits the station in a short launch animation.

Use the map to choose one of the universe's 24 planets. Defeat all orbital waves to secure a world; every fourth planet has a guardian fleet. Once the orbit is clear, approach within 650 m of the planet surface, slow below 120 m/s, and press **L** to land. Each planet has a generated, bounded landing region with terrain, vegetation or crystals, ruins, and eight collectible items. Exploration takes place on these landing sites rather than across the entire planetary sphere. Walk to glowing items and press **E** to collect alloy, crystals, and relics. They grant upgrade credits immediately; the workshop also accepts item trades. Collected items do not respawn. Cleared worlds also offer repeatable orbital patrol challenges with **T**. The landing corridor stays closed until all patrol waves are defeated; your secured-world progress is preserved.
...
Secure all 24 planets in the current universe, then summon its guardian from the map while in a secured orbit. A rift arrival animation introduces the boss. Defeat it to enter a cinematic black-hole crossing: a bright accretion disk, approach to the event horizon, and a luminous tunnel before losing consciousness. You wake in the next station's medical bay, with progress, inventory, and upgrades intact. Space skips the crossing; E gets you out of the recovery bed once you have opened your eyes. Closing the game mid-crossing resumes safely in medical. The sixth guardian is the Null Sovereign: its defeat ends the story with all 144 worlds freed. A failed or interrupted boss encounter can be attempted again.
...
| Shift | Cruise | Run |
| Space | Fire | Jump |
| E | Nova pulse | Talk to crew / board ship / collect item |
| B | Brake (overrides thrust, strafe, cruise, autopilot) | — |
| Q / R | Descend / ascend | — |
| F | Toggle autopilot | — |
| H | Plot course to base | — |
| T | Challenge a new patrol at a secured world | — |
| L | Land / dock | — |
...
Approach a crew member and press **E** to talk. Choose dialogue topics with the mouse; Escape or Back to exploring closes the conversation. Jun Park, the engineer, opens your workshop. Commander Vale assigns an unsecured world. Dr. Noor accepts one relic for 180 research credits per universe. Dr. Imani gives three alloy supply packs once per universe. Mika Reyes shares crew stories and explains the gate network. The station directory shows their locations and distances.
...
Run `node tests/campaign.cjs` and `node tests/flight-save.cjs` for campaign/save and flight regression checks. `tests/browser-journey.cjs` exercises walking, boarding, combat-gated landing, loot, docking, saves, all six boss transitions, and the ending in Chrome. `tests/browser-surfaces.cjs` checks all six surface styles, pause/resume, item trading, and upgrades. `tests/browser-crew-crossing.cjs` checks connected station routes, every crew conversation, repeatable patrols, black-hole rendering, waking up, and interrupted-save recovery. Browser tests use `playwright-core` supplied via `PLAYWRIGHT_MODULE`, an installed browser via `CHROME_PATH`, and a local server via `STARFARER_URL` (defaults documented in the script).
